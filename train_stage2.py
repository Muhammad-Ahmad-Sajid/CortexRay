...
def train_one_epoch(
    ...
    for idx, batch in enumerate(loader):
        ...
        if (idx + 1) % 10 == 0 or (idx + 1) == len(loader):
            print(f"  Batch {idx+1}/{len(loader)} - Loss: {total_loss.item():.4f} | Frac Acc: {100.0*correct_frac/total:.2f}% | Region Acc: {100.0*correct_region/total:.2f}%")
            gc.collect()
        ...
        if save_freq > 0 and (idx + 1) % save_freq == 0 and (idx + 1) < len(loader) and checkpoint_dir is not None:
            ...
            print(f"  [Checkpoint] Saved mid-epoch state at Batch {idx+1}/{len(loader)} to: {latest_checkpoint_path.name}")
            gc.collect()
...
def main():
    ...
    parser.add_argument("--csv_path", type=str, default="d:/X-ray ML Model/stage2_labels.csv", help="Path to Stage 2 CSV labels")
    parser.add_argument("--resume", action="store_true", help="Resume training from the last saved latest checkpoint")
    ...
    if args.resume:
        if latest_checkpoint_path.exists():
            ...
            if batch_idx is not None:
                ...
                print(f"[*] Successfully loaded mid-epoch checkpoint. Resuming from Epoch {start_epoch}, Batch {start_batch}")
            else:
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        print(f"  -> Train Loss: {train_loss:.4f} | Frac Acc: {train_acc_frac*100.0:.2f}% | Region Acc: {train_acc_region*100.0:.2f}%")
        ...
        print(f"  -> Val Loss:   {val_loss:.4f} | Frac Acc: {val_acc_frac*100.0:.2f}% | Region Acc: {val_acc_region*100.0:.2f}%")
