        ...
        for label_name, label_idx in [("normal", 0), ("abnormal", 1)]:
            ...
            if class_dir.exists():
                for file in class_dir.iterdir():
                    if file.suffix.lower() in [".png", ".jpg", ".jpeg"] and not file.name.startswith("._"):
...
# ------------------------------------------------------------------------------
def train_epoch(model, loader, criterion, optimizer, device, epoch, start_batch=0, save_freq=500, checkpoint_dir=None, best_val_loss=None, early_stop_counter=None):
    model.train()
    running_loss = 0.0
    correct = 0
    total = 0
...
def main():
    ...
    parser.add_argument("--resume", action="store_true", help="Resume training from the last saved best checkpoint")
            ...
            for k, v in state_dict.items():
                if k in model_state:
                    if v.shape == model_state[k].shape:
                        ...
                        print(f"[*] Skipping key {k} due to shape mismatch: checkpoint shape {v.shape} vs model shape {model_state[k].shape}")
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        train_loss, train_acc = train_epoch(
            ...
            epoch=epoch, start_batch=current_start_batch, save_freq=500,
            checkpoint_dir=checkpoint_dir, best_val_loss=best_val_loss,
