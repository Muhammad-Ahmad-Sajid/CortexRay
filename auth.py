...
def require_doctor(current_user: User = Depends(get_current_user)) -> User:
    if current_user.role not in ["doctor", "admin"]:
        logger.warning(f"Access denied: User {current_user.email} (role: {current_user.role}) attempted to access a doctor route.")
        raise HTTPException(
...
def require_admin(current_user: User = Depends(get_current_user)) -> User:
    if current_user.role != "admin":
        logger.warning(f"Access denied: User {current_user.email} (role: {current_user.role}) attempted to access an admin route.")
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
...
if __name__ == "__main__":
    ...
    try:
        ...
        if decoded_payload.get("sub") == "doctor@hospital.com" and decoded_payload.get("role") == "doctor" and "exp" in decoded_payload:
